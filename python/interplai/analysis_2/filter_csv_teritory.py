...
score_dict={"Teritory":["Country","Total Entry","Scores","score %"]}
...
for teritory_name in country_list:
    ...
    mycountry = list(mydata['country'].unique())
    # print(mycountry)
    ...
    for column_name in df_after_removing_nan.columns:
        ...
        if column_name in score_matrix:
            ...
            # print(teritory_name,column_name, factor)
    ...
    # score_dict[teritory_name]=[total_entries, round(scores,4)]
    ...
    #base folder path
    base_path="territory"
    pdf_filename=base_path+"/"+teritory_name+"_"+str(total_entries)+".csv"
    # check if path exist
