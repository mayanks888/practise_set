...
index=data.groupby("country").count().vertical_id
...
score_dict={"Country":["Total Entry","Scores", "Scores%"]}
# score_dict={"country":["Country","Total Entry","Scores"]}
...
for country_name in country_list:
    ...
    mydata=data.copy()
    bblabel = []
    mydata.drop(mydata.loc[mydata['country']!=country_name].index, inplace=True)
    ...
    # score_dict[country_name]=[total_entries, round(scores,4)]
...
df_rank.to_csv(base_path+'/Ranking_country.csv',index=True)